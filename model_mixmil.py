...
class MixMIL(torch.nn.Module):
    ...
    def __init__(self, Q, K, P=1, embed_dim=1024, likelihood="binomial", n_trials=2, mean_field=False, init_params=None):
        ...
        super().__init__()
        self.Q = Q
        self.mlp = nn.Sequential(nn.Linear(embed_dim, Q), nn.ReLU())
    ...
    def init_with_mean_model(Xs, F, Y, likelihood="binomial", n_trials=None, mean_field=False):
        ...
        print(f"Initializing MixMIL with mean model: Q={Q}, K={K}, P={P}")
    ...
    def likelihood(self, logits, y):
        if self.likelihood_name == "binomial":
            ...
            y = y[:, None].expand(-1, logits.shape[1])  # [B] -> [B,S]
    ...
    def forward(self, Xs, n_samples=8, scaling=None, predict=False):
        Xs = [self.mlp(x) for x in Xs] if torch.is_tensor(Xs) else [self.mlp(x) for x in Xs]
    ...
    def train(self, X, F, Y, n_epochs=2_000, batch_size=64, lr=1e-3, verbose=True, device=None):
        ...
        history = []
        for epoch in trange(1, n_epochs + 1, desc="Epoch", leave=True, disable=not verbose):
            bar = tqdm(train_loader, desc=f"e{epoch}", leave=True, disable=not verbose)
            for step, (xs, f, y) in enumerate(bar):
                if device:
                    xs, f, y = to_device((xs, f, y), device)
                u = self(xs)
                ...
                bar.set_postfix(loss=f"{ldict['loss']:.4f}", ll=f"{ldict['ll']:.4f}", kld=f"{ldict['kld']:.4f}")
                # bar.refresh()
                # if step % 50 == 0: tqdm.write(f"e{epoch} s{step} | loss {ldict['loss']:.4f}")
